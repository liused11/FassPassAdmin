  ...
  export class InboxComponent implements OnInit {
    ...
    openHistory(building: any) {
      ...
      // 🔹 โหลด history ของ building นั้น
      this.loadHistory(building.id);
    }
    loadHistory(buildingId: string) {
      this.loading = true;

      const mockData: Record<string, ParkingHistoryItem[]> = {
        '1': [
          {
            type: 'status',
            title: 'แก้ไขสถานะ',
            description: 'เปลี่ยนเป็น เปิดใช้งาน',
            time: '5 นาทีที่แล้ว',
            icon: 'pi pi-car',
            color: 'blue'
          }
      ...
      this.historyList = mockData[buildingId] ?? [];
