...
export interface UserProfile { id: string; name: string; email: string; phone: string; avatar: string; role: string; created_at: string; joined_date: string; }
...
export class UserManagementService {
    ...
    // ✅ เพิ่ม URL ของฟังก์ชัน Invite
    private inviteUrl = 'https://unxcjdypaxxztywplqdv.supabase.co/functions/v1/invite-admin-auth';
    ...
    // ✅ ฟังก์ชันใหม่: ส่งคำเชิญผ่าน Edge Function
    inviteUser(token: string, email: string): Observable<any> {
        const headers = new HttpHeaders({ 
            Authorization: `Bearer ${token}`, 
            apikey: this.anonKey,
            'Content-Type': 'application/json'
        });
        return this.http.post<any>(this.inviteUrl, { email }, { headers });
    }
}